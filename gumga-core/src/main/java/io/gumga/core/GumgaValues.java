...
public interface GumgaValues {
    ...
    default Properties getCustomFileProperties() {
        ...
        try(InputStream input = new FileInputStream(System.getProperty("user.home") + "/gumgafiles/" + getCustomPropertiesFileName());) {
            toReturn.load(new InputStreamReader(input, Charset.forName("UTF-8")));
        } catch (IOException e) {
